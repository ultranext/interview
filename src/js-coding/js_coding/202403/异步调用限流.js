/**
 * JS的promise可用于封装各类异步调用场景，但多数接口都有访问qps的限制，
 * 要求实现一个ES6 class (或ES5函数类），
 * 该类可以接受并行执行promise的上限参数M作为构造参数，
 * 通过提供一个enqueue方法来接收promise生成器，
 * 并确保线程执行中的promise数量始终不超过M，调用样例参考：
 * 
